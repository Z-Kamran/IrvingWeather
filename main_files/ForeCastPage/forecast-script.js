...
.then(res=>{
    ...
    for(let i=0; i<dayNode.length; i++){
       let day =  new Date(dateData[i]).getDay();
        dayNode[i].innerHTML = daysOfTheWeek[day]
    }
    ...
    const sunRise = res.data.daily.sunrise[3] // as the sunrise and sunset time was not accurate or maybe I didn't undrestand it, 
    const sunSet = res.data.daily.sunset[3] // I sticked with one day of the week for the rest fo the week.
    ...
    // As the time for the sunrise was not accurate on API, made some changes to correct it. (it was always five hours ahead)
    ...
    // As the time for the sunset was not accurate on API, made some changes to correct it. (it was always sesven hours behind)
...
call() // invoking the function and undating the get request every 10 seconds.
setInterval(()=>call(),
10000)
